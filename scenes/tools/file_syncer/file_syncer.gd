...
signal sheet_finished(sheet_name: String)
...
func _on_http_request_request_finished(_result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
...
func _save_csv_and_json(body: PackedByteArray) -> void:
	...
	sheet_finished.emit(current_sync)
