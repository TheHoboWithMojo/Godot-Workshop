	...
	for sheet_name in spreadsheet_dict:
		if not _is_static(sheet_name):
			...
			if not success_current:
				...
				# Don't clean up - we might need the temp and backup
				continue
...
func get_data_properties(sheet_name: String) -> Array:
	if _sheet_exists(sheet_name):
		var data_path: String = spreadsheet_dict[sheet_name]["data_path"]
		var data: Dictionary = load_json_file(data_path)
		if data.is_empty():
			return []
		
		if data.size() > 0:
			var first_item_key = data.keys()[0]
			return data[first_item_key].keys()
